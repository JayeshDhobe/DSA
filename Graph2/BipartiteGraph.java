...
public class BipartiteGraph {
    ...
    public static boolean isBipartite(ArrayList<Edge>[] graph) { // time comp-O(V+E), space comp O(v[vis]) + O(v[col]) =
                                                                 // O(V)
            ...
            if (col[i] == -1) { // MBFS
                ...
                while (!q.isEmpty()) {
                    ...
                    for (int j = 0; j < graph[curr].size(); j++) {
                        ...
                        if (col[e.dest] == -1) {
                            int nextCol = col[curr] == 0 ? 1 : 0; // agar curr ka color 0 hai to next iska opposite 1
                                                                  // hoajeag agar 1 ha to 0 hoga
